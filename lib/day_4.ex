defmodule Aoc2018.Day4 do
  def part_1() do
    {_curr_action, guards} =
      ...
      |> Enum.reduce({%{}, %{}}, fn %{"action" => action, "minutes" => minute},
                                    {curr_action, results} ->
        handle_action(action, minute, curr_action, results)
      end)

    {sleepy_guard_id, {_total_sleep_time, mins_asleep}} =
      guards
      |> Enum.max_by(fn {_guard_id, {total_slept, _mins_slept}} -> total_slept end)

    {most_mins_slept, _times_asleep} =
      mins_asleep
  ...
  defp handle_action(
         "f" <> _rest = _action,
         min,
         %{"guard_id" => _guard_id} = curr_action,
         results
       ) do
  ...
  defp handle_action(
         "w" <> _rest = _action,
         min,
         %{"guard_id" => guard_id, start_sleep: start_sleep} = _curr_action,
         results
       ) do
    ...
    initial_mins =
      Enum.reduce(minutes_slept, %{}, fn x, acc ->
        Map.put(acc, x, 1)
      end)

    {%{"guard_id" => guard_id},
     Map.update(results, guard_id, {sleep_time, initial_mins}, fn {total_sleep_time,
                                                                   curr_slept_mins} ->
       new_slept_time = total_sleep_time + sleep_time

       new_mins_slept =
         Enum.reduce(minutes_slept, curr_slept_mins, fn min, acc ->
           Map.update(acc, min, 1, fn m ->
             m + 1
           end)
         end)

       {new_slept_time, new_mins_slept}
     end)}
  end
